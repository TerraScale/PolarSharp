...
using Polar.NET.Models.Files;
using File = Polar.NET.Models.Files.File;
...
public class FilesApi
{
    ...
    public async Task<PaginatedResponse<File>> ListAsync(
        int page = 1,
        int limit = 10,
        CancellationToken cancellationToken = default)
    {
        ...
        return JsonSerializer.Deserialize<PaginatedResponse<File>>(content, _jsonOptions)
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var content = await response.Content.ReadAsStringAsync(cancellationToken);
        return JsonSerializer.Deserialize<File>(content, _jsonOptions)
            ?? throw new InvalidOperationException("Failed to deserialize response.");
    }
    ...
    public async IAsyncEnumerable<File> ListAllAsync([EnumeratorCancellation] CancellationToken cancellationToken = default)
    {
        var page = 1;
